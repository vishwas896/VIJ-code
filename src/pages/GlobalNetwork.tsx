...
export const GlobalNetwork: React.FC = () => {
  ...
  const [connections, setConnections] = useState<{ connection_id: number; status: 'connected' | 'requested_sent' | 'requested_received'; created_at: string }[]>(() => {
    const saved = localStorage.getItem(`vij_connections_${userId}`);
  ...
  const [preferences, setPreferences] = useState(() => {
    const saved = localStorage.getItem(`vij_network_preferences_${userId}`);
    return saved ? JSON.parse(saved) : {
  ...
  const [bookmarkedIds, setBookmarkedIds] = useState<number[]>(() => {
    const saved = localStorage.getItem(`vij_bookmarks_${userId}`);
    return saved ? JSON.parse(saved) : [];
  });
